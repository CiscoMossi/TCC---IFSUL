...
export class HomeScreen extends Component {
  ...
  showComments = postId => {
    const currentPost = this.state.feed.find(post => post._id === postId)
  ...
  render() {
    const { feed, currentPost } = this.state
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
          { feed.map((post, index) => {
            ...
            if (post.type === 'MEDITATION') {
              postProps.link = postService.getMeditationAudio(post._id)
              postProps.title = post.title
            }
